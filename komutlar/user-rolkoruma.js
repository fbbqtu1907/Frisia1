...
let veri = await db.fetch(`rolkoruma_${message.guild.id}`) 
let geçerli = args[0]

if(geçerli === 'on') {
 
  if(veri) return message.channel.send("Looks like the system's already active.")
  message.channel.send('Okay! Role protection system is currently active')
  db.set(`rolkoruma_${message.guild.id}`, 'on') 
message.guild.owner.send('Role protection system actived by `'+message.author.username+'`')
}

if(geçerli === 'off') {
 
  if(!veri) return message.channel.send('Apparently the system is already disabled.')
  message.channel.send('Okay! Role protection system is currently disabled')
   db.delete(`rolkoruma_${message.guild.id}`)
  message.guild.owner.send('Role protection system disabled by `'+message.author.username+'`')
}
